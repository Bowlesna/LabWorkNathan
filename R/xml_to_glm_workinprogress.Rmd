...
First test, a test to setup the GLM model using every ear. Some ears that are initially loaded disapear because they are faulty .xml files.
...
#turn the data into a glm
glm_data = lapply(bin_data, function(x) {glm(cbind(GFP,WT) ~ bins+bins_squared,family = quasibinomial(link = "logit"), data = x)})
...
```{r}
#load file names based on your folder
filenames = list.files("../Data_for_Analysis/strange_data", pattern="*.xml", full.names=TRUE)
